...
import MarkdownUpImgView from './markdown-up-img-view';
...
export default {
  ...
  activate(state) {
    this.markdownUpImgView = new MarkdownUpImgView(state.markdownUpImgViewState);
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'markdown-up-img:toggle': () => this.toggle(),
      'markdown-up-img:upImg': () => this.upImg(),
      'markdown-up-img:stark': () => this.stark()
    }));
  },
  ...
  toggle() {
    console.log('MarkdownUpImg was toggled!');
    return (
  ...
  upImg() {
    ...
    if (atom.config.get('markdown-up-img.only_markdown')) {
    ...
    filename = atom.config.get('markdown-up-img.filePrefix') + "-" + (new Date().format()) + ".png";
    curDirectory = dirname(cursor.getPath());
    fullname = join(curDirectory, filename);
    if (atom.config.get('markdown-up-img.use_assets_folder')) {
    ...
    if (atom.config.get('markdown-up-img.upload_to_mssm')) {
    ...
    if (!atom.config.get('markdown-up-img.upload_to_qiniu')) {
      mdtext = '![](';
      if (atom.config.get('markdown-up-img.use_assets_folder')) {
        mdtext += 'assets/';
      }
      ...
      qiniu = require('qiniu');
      qiniu.conf.ACCESS_KEY = atom.config.get('markdown-up-img.zAccessKey');
      qiniu.conf.SECRET_KEY = atom.config.get('markdown-up-img.zSecretKey');
      bucket = atom.config.get('markdown-up-img.zbucket');
      domain = atom.config.get('markdown-up-img.zdomain');
      key = filename;
      ...
      filePath = fullname;
      uphost = atom.config.get('markdown-up-img.zuphost');
      if (uphost) {
